...
//变更单中的数据变更单
RegularsExp.dataFile = /(^(NCRM配置变更单|NCRM模型变更单|NCRM数据变更单|智慧BSS配置变更单|智慧BSS模型变更单|智慧BSS数据变更单)-([\u4e00-\u9fa5]|[\x00-\xff])+((.txt)$|(.sql)$|(.SQL)$|(.xlsx)$|(.xls)$))/g;
//变更单中的测试报告
RegularsExp.testReporter = /^(测试报告)([\u4e00-\u9fa5]|[\x00-\xff])+((.doc)$|(.docx)$)/g;
//变更单中的开发变更单
RegularsExp.devOrder = /^(NCRM开发变更单|智慧BSS开发变更单)([\u4e00-\u9fa5]|[\x00-\xff])+((.xls)$|(.xlsx)$)/g;
